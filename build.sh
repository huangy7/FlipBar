...
# FlipBar One-Click Universal DMG Builder
# --------------------------------------
...
if [ "$ARCH" == "arm64" ]; then
    xcodebuild build -scheme "${APP_NAME}" -configuration Release -derivedDataPath "${DERIVED_DATA}" ARCHS="arm64" ONLY_ACTIVE_ARCH=NO
elif [ "$ARCH" == "x86_64" ]; then
    xcodebuild build -scheme "${APP_NAME}" -configuration Release -derivedDataPath "${DERIVED_DATA}" ARCHS="x86_64" ONLY_ACTIVE_ARCH=NO
else
    xcodebuild build -scheme "${APP_NAME}" -configuration Release -derivedDataPath "${DERIVED_DATA}" ARCHS="arm64 x86_64" ONLY_ACTIVE_ARCH=NO
fi
...
SRC_APP_PATH="${DERIVED_DATA}/Build/Products/Release/${APP_NAME}.app"
